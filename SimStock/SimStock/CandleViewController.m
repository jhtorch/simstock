...
- (void)viewDidLoad {
	...
	//search bar
	searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(self.view.frame.size.width-250, 0, 250, 40)];
	...
	//candleChart
	self.candleChart = [[CandleChart alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-60)];
	[self.view addSubview:candleChart];
	
    
    //button
    
    self.backButton = [UIButton buttonWithType:UIButtonTypeInfoLight];
    [self.backButton setFrame:CGRectMake(3, 3, 25, 15)];
    [self.backButton setTitle:@"Back" forState:0];
    [self.backButton setTitle:@"Back" forState:1];
...
-(void) reload:(NSString *)name;
{
    searchBar.text = name;
    self.req_security_id = name;
	[self searchBarSearchButtonClicked:searchBar];
}
