	...
	for(int i=0;i<self.sections.count;i++){
		...
		for(int s=0;s<sec.series.count;s++){
			...
			for(int j=0;j<label.count;j++){
				...
				[text drawAtPoint:CGPointMake(sec.frame.origin.x+sec.paddingLeft+2+w,sec.frame.origin.y) withFont:[UIFont systemFontOfSize: 9]];
				w += [text sizeWithFont:[UIFont systemFontOfSize:9]].width;
			}
			[label release];
		}
	}
...
-(void)drawYAxis{
	...
	for(int secIndex=0;secIndex<self.sections.count;secIndex++){
		...
		for(int aIndex=0;aIndex<sec.yAxises.count;aIndex++){
			...
			[[@"" stringByAppendingFormat:format,yaxis.baseValue] drawAtPoint:CGPointMake(sec.frame.origin.x-1,baseY-7) withFont:[UIFont systemFontOfSize: 9]];
			...
			for(int i=1; i<= yaxis.tickInterval+1;i++){
				if(yaxis.baseValue + i*step <= yaxis.max){
					...
					[[@"" stringByAppendingFormat:format,yaxis.baseValue+i*step] drawAtPoint:CGPointMake(sec.frame.origin.x-1,iy-7) withFont:[UIFont systemFontOfSize: 9]];
