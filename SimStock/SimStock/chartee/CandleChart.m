...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		self.backgroundColor = [[[UIColor alloc] initWithRed:20/255.f green:20/255.f blue:20/255.f alpha:1] autorelease];
    }
    return self;
}
	...
	if([type isEqualToString:@"candle"]){
		for(int i=self.rangeFrom;i<self.rangeTo;i++){
			...
			if(i == self.selectedIndex && self.selectedIndex < data.count && [data objectAtIndex:self.selectedIndex]!=nil){
				...
				CGSize size = [[category objectAtIndex:self.selectedIndex] sizeWithFont:[UIFont fontWithName:@"Helvetica" size:9.0]];
				...
				[[category objectAtIndex:self.selectedIndex] drawAtPoint:CGPointMake(x+2,y+1) withFont:[UIFont fontWithName:@"Helvetica" size:9.0]];
				CGContextSetShouldAntialias(context, NO);	
			}
		}
	}
